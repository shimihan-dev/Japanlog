...
import { RefreshCw, Calculator, CloudSun, ShoppingBag, Sparkles, Wind, Droplets } from "lucide-react";
...
export const TravelUtilityWidget: React.FC<TravelUtilityWidgetProps> = ({ selectedCode }) => {
  const [activeTab, setActiveTab] = useState<"currency" | "weather">("currency");
  ...
  const handleJpyChange = (val: string) => {
    ...
    const effectiveJpy = isTaxFree ? num / 1.1 : num;
    const convertedKrw = Math.round(effectiveJpy * rateData.jpyToKrw);
  ...
  const handleKrwChange = (val: string) => {
    ...
    const finalJpy = isTaxFree ? Math.round(convertedJpy * 1.1) : convertedJpy;
    setJpyInput(finalJpy.toString());
  };
  ...
  return (
    <div className="bg-white dark:bg-[#151D2A] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-4 transition-colors duration-200 space-y-3">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl gap-1">
          <button
            ...
            className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              activeTab === "currency"
                ? "bg-white dark:bg-slate-700 text-blue-600 dark:text-cyan-300 shadow-2xs"
      ...
      {/* Tab 1: Currency Exchange Converter */}
      {activeTab === "currency" && (
        <div className="space-y-3 animate-in fade-in duration-150">
          ...
          <div className="flex items-center justify-between text-xs bg-blue-50/80 dark:bg-slate-800/80 px-3 py-2 rounded-xl border border-blue-100 dark:border-slate-700/80">
            <span className="font-bold text-blue-900 dark:text-blue-200">
              현재 환율: 100 JPY ➔ <span className="text-blue-600 dark:text-cyan-400 font-extrabold">{rateData?.rate100Jpy ?? 895} KRW</span>
            </span>
          ...
          {/* Tax-Free Toggle */}
          <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/60 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800 text-xs">
            <button
              onClick={() => {
                const nextState = !isTaxFree;
                setIsTaxFree(nextState);
                handleJpyChange(jpyInput);
              }}
            ...
            >
              ...
              <span>🛍️ 10% 현지 면세(Tax-Free) 적용 계산</span>
            </button>
            <input
              type="checkbox"
              checked={isTaxFree}
      ...
      {/* Tab 2: Live City Weather & Outfit Tips */}
      {activeTab === "weather" && (
        <div className="space-y-3 animate-in fade-in duration-150">
          {weatherData ? (
